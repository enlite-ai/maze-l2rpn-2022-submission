...
class SerializedTorchPolicyP36(TorchPolicy):
    ...
    def __init__(self, model: Union[DictConfig, Dict], state_dict_file: str, spaces_dict_file: str, device: str):
        ...
        observation_spaces_dict = {key: gym.spaces.Dict({key2: gym.spaces.Box(low=vv.low, high=vv.high, dtype=vv.dtype) for key2, vv in value.spaces.items()})
                                   for key, value in spaces_config.observation_spaces_dict.items()}
